
REPORT GENERATED: 2026-08-25

# Zines / Artist Books — Battle Plan

## 1. Quick Judgment
**Fit score:** 6.2/10  
**Fit band:** Promising  
**Confidence:** High  
**Type:** Print / Zines / Bookstores  
**City:** Tokyo  

## 2. Verification Status
Verified: Deadline, Fees. Needs checking: Official/source website, Submission process, Contact.

- **Official/source website**: No source attached (missing)
- **Submission process**: Not publicly listed (needs_inquiry)
- **Deadline**: Rolling — check website for current window (verified)
- **Fees**: Estimated print test: ¥10,000–¥25,000 (verified)
- **Contact**: Not publicly listed (needs_inquiry)

## 3. Why This May Fit the Artist
- Opportunity type: Print / Zines / Bookstores.
- Artist themes to compare against this venue: urban atmosphere, domestic stillness, cats, interior light, memory, quiet life.
- Visual-language fit signals: still, intimate, atmospheric, nostalgic without sentimentality, attentive, quietly melancholic.

## 4. Public Opportunity Summary
Fast, low-cost path with 12 curated targets, 6 high-priority targets, and 7 Koenji/Nakano targets.

## 5. Existing Fit Notes
This path fits quiet works-on-paper and visual sequencing because one printed object can become a shop placement, fair application, portfolio proof, and publisher conversation starter.

## 6. Key Points
- Known curated targets: 12.
- Phase 1 route: Koenji + Nakano.
- Success condition: two finished zines, five outreach attempts, one accepted placement.

## 7. Submission / Logistics
- **Deadline:** Rolling — check website for current window
- **Fees:** Estimated print test: ¥10,000–¥25,000
- **Submission page:** Not publicly listed
- **Source:** No source attached

## 8. Risk / Unknowns
No hard dealbreaker recorded. Manual verification recommended.

## 9. Recommended Next Step
Start with Tacoche, LOCAL Gallery・Books, Dig A Hole Zines, 本店・本屋の実験室, and そぞろ書房.

## 10. Inquiry Need
If submission/contact details are missing, generate a short inquiry email instead of presenting the opportunity as fully verified.

_Last generated: 2026-08-25_

