
REPORT GENERATED: 2026-08-21

# Gallery IRO

## 1. Quick Judgment
**Fit score:** 7.1/10  
**Fit band:** Strong fit  
**Confidence:** Medium  
**Type:** Other  
**City:** City not listed  

## 2. Verification Status
Verified: Official/source website, Deadline. Needs checking: Submission process, Fees, Contact.

- **Official/source website**: https://1-6.jp/ (verified)
- **Submission process**: Not publicly listed (needs_inquiry)
- **Deadline**: 2025-12-27 (verified)
- **Fees**: Not publicly listed (needs_inquiry)
- **Contact**: Not publicly listed (needs_inquiry)

## 3. Why This May Fit the Artist
- Opportunity type: Other.
- Artist themes to compare against this venue: urban atmosphere, domestic stillness, cats, interior light, memory, quiet life.
- Visual-language fit signals: still, intimate, atmospheric, nostalgic without sentimentality, attentive, quietly melancholic.

## 4. Public Opportunity Summary
Gallery IRO is a local gallery source in Tokyo for watercolor / works on paper research.

## 5. Existing Fit Notes
Gallery IRO's focus on watercolor and works-on-paper research is in your medium and daily practice, while their emphasis on the medium itself rather than trends makes it an ideal venue for your sustained architectural observation work. Your approach to capturing fleeting urban moments in Tokyo through watercolor would resonate with a gallery dedicated to the specificity of paper-based techniques.

## 6. Key Points
- Check the official website for current submission guidelines and open call status
- Verify current status at the official site before investing time — websites change and open calls close early
- Contact information or submission guidelines may require direct research

## 7. Submission / Logistics
- **Deadline:** 2025-12-27
- **Fees:** Not publicly listed
- **Submission page:** Not publicly listed
- **Source:** https://1-6.jp/

## 8. Risk / Unknowns
No hard dealbreaker recorded. Manual verification recommended.

## 9. Recommended Next Step
Verify current submission, exhibition, or contact details before recommending.

## 10. Inquiry Need
If submission/contact details are missing, generate a short inquiry email instead of presenting the opportunity as fully verified.

_Last generated: 2026-08-21_

