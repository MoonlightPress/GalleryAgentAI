...
[artist name]""",
        ...
        for section_name, categories in section_map.items():
            ...
            visible = section_opps[:8]

            cols = st.columns(4)

            for idx, opp in enumerate(visible[:4]):
            ...
            if len(visible) > 4:
                ...
                with st.expander("Show more in this section"):

                    more_cols = st.columns(4)

                    for offset, opp in enumerate(visible[4:8]):

                        with more_cols[offset]:
                            ...
                            with st.container(border=True):
                                ...
                                if st.button(
                                    "More",
                                    key=f"more_extra_{section_name}_{offset}_{title}"
                                ):
                                    st.session_state["selected_opportunity"] = opp
                                    st.session_state["selected_section"] = section_name
                                    st.rerun()
...
print("Patched Opportunities tab safely without raw HTML.")