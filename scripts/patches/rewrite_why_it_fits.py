"""
rewrite_why_it_fits.py

Batch rewrites weak why_this_fits_short fields in compact_opportunities.json
using Claude Haiku. Targets entries in visible buckets where the why field is:
...
Usage:
    python scripts/patches/rewrite_why_it_fits.py
"""
import sys
import json
import os
...
# In a git worktree, the actual shared repo .env may be in the parent's parent
...
def is_weak(opp: dict) -> tuple:
    """Return (is_weak, reason) for a given opportunity's why_this_fits_short."""
    ...
    # Check if why is just a minor word-shuffle of one_sentence
...
- Do NOT use "watercolor artistly" or similar garbled phrases — use "watercolor" or "works on paper"
...
def main():
    ...
    if not api_key:
        ...
        try:
            ...
            pass
    if not api_key:
        # Manual .env reading — check root and shared repo
    ...
    # Find weak entries in target buckets
    targets = []
    ...
    print(f"Found {len(targets)} entries with weak why_this_fits_short in target buckets.")
    print()
    ...
    for idx, (opp_idx, opp, reason) in enumerate(targets, 1):
        ...
        try:
            ...
            # Sanity check: must be non-empty and not too short
    ...
    print()
    print(f"Done. {updated}/{len(targets)} entries updated. {errors} errors.")
    print(f"Saved to: {OPP_PATH}")
