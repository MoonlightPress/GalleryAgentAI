...
export const STATES = {
  ...
  a_motion: {
    label: T('The work seen in motion', '被看见的作画过程', '描いているところが見える'),
    detail: T('Watercolour spreading into wet paper is one of the very few things a still image cannot show, and it is the single change on this ladder that asks for no extra painting — the camera watches work already being done.',
              '水彩在湿纸上化开，是静止图像几乎唯一拍不到的东西；而且这是这条阶梯上唯一不要求她多画一张的改变——镜头拍的，是本来就在做的事。',
              '濡れた紙に水彩がにじむ瞬間は、静止画がほぼ唯一見せられないもの。しかもこの梯子で唯一、余分に描くことを求めない変化——カメラはすでにしている仕事を見ているだけ。'),
    treatment: {
      open: T('Why this one matters more than the rest', '为什么这一级比其它几级都重要', 'なぜこれが他より効くのか'),
      hide: T('Close', '收起', '閉じる'),
      blocks: [
        { label: T('What it changes', '它改变了什么', '何が変わるか'),
          text: T('A finished painting competes with every other finished painting. A painting being made competes with almost nothing, because the moment pigment hits wet paper cannot be screenshotted, described, or reproduced by anyone else. It is the one thing in her practice that is structurally hers.',
                  '一张画完的画，要和所有画完的画竞争。一张正在被画的画几乎没有对手——颜料落在湿纸上的那一瞬间，没法截图、没法转述，也没有别人能复制。这是她的实践里，结构上唯一属于她自己的东西。',
                  '描き上がった絵は、描き上がった他のすべてと競う。描かれている絵はほとんど競わない。顔料が濡れた紙に落ちる瞬間は、切り取ることも、言い換えることも、誰かが再現することもできないから。彼女の実践の中で、構造的に彼女だけのもの。'),
        },
        { label: T('Why it costs nothing', '为什么它没有额外成本', 'なぜ負担にならないか'),
          text: T('Every other state on this ladder asks for something new — a series to invent, a platform to learn, a person to write to. This one asks for a phone on a stand pointing at a desk she is already sitting at, for work she is already doing daily. The material is a by-product.',
                  '这条阶梯上其它每一级都要求一件新的事——想一个系列、学一个平台、给一个人写信。这一级只要求一部架好的手机，对着她本来就坐着的桌子，拍她本来每天都在做的事。素材是副产品。',
                  'この梯子の他はすべて新しい何かを求める——連作を考える、場所を覚える、誰かに書く。これはスタンドに置いた携帯を、すでに座っている机に向けるだけ。すでに毎日している仕事の副産物として素材が出る。'),
        },
        { label: T('What is worth filming', '值得拍的是什么', '撮る価値があるのは'),
          text: T('Not the whole painting. The moments where the medium does something a viewer cannot predict — a wash spreading into damp paper, salt or a dry brush breaking a flat area, the first dark going into a pale one, lifting colour back out with a clean brush. Architecture gives her hard edges to cut against soft ones, which is the contrast that holds attention.',
                  '不是整张画。是媒介做出观众预料不到的事的那些瞬间——一片洗染在潮纸上化开、盐或干笔破开一块平涂、第一笔重色落进浅色里、用干净笔把颜色再吸出来。建筑给了她硬边，去和软边形成对比——留住注意力的正是这种反差。',
                  '絵の全部ではない。画材が見る側の予測を超える瞬間——にじみが湿った紙に広がるところ、塩や乾いた筆が平らな面を壊すところ、淡い中に最初の濃い色が入るところ、きれいな筆で色を抜き戻すところ。建築は硬い輪郭を与え、それが柔らかいにじみと対比する。その落差が注意を保つ。'),
        },
        { label: T('Where it goes', '它发到哪里', 'どこに出すか'),
          text: T('The same file works everywhere without re-editing, which is what makes it the natural partner to a second audience elsewhere — one recording, several places, no translation, because nobody needs language to watch paint move.',
                  '同一个文件到哪儿都能用，不用重剪——所以它天然是「在别处的第二群受众」那一级的搭档：拍一次，发几个地方，不需要翻译，因为看颜料流动不需要语言。',
                  '同じ素材が編集し直さずどこでも使える。だから「別の場所のもう一つの観客」と組むのが自然。一度撮って、複数の場所へ。絵具が動くのを見るのに言葉はいらない。'),
        },
        { label: T('Where this sits', '它在什么位置', 'これはどこに効くか'),
          text: T('It is not only an audience state. A named series becomes far easier to sustain when each entry has a second life as a recording, and a brand deciding whether to license a look is reassured by seeing it made by hand.',
                  '它不只属于受众这一条。当每一张画都还有「一段录像」这第二次生命时，一个有名字的系列会容易维持得多；而正在考虑授权的品牌，看到这种面貌是手工做出来的，会更放心。',
                  '観客のためだけの段階ではない。一枚ごとに映像という第二の生があれば、名前のある連作ははるかに続けやすい。ライセンスを検討するブランドも、手で作られていると見えれば安心する。'),
        },
      ],
    },
  },
