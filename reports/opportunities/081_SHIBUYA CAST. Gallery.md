# SHIBUYA CAST. Gallery

## 1. Quick Judgment
**Fit score:** 10/10  
**Fit band:** Excellent fit  
**Confidence:** High  
**Type:** Cafe / Local Wall Spaces  
**City:** Tokyo — Shibuya  

## 2. Verification Status
Core public information appears present.

- **Official/source website**: https://shibuyacast.jp/ (verified)
- **Submission process**: https://shibuyacast.jp/ (verified)
- **Deadline**: ongoing — proposal-based (verified)
- **Fees**: Confirm with venue (verified)
- **Contact**: @shibuyacast (verified)

## 3. Why This May Fit the Artist
- Opportunity type: Cafe / Local Wall Spaces.
- Artist themes to compare against this venue: urban atmosphere, domestic stillness, cats, interior light, memory, quiet life.
- Visual-language fit signals: still, intimate, atmospheric, nostalgic without sentimentality, attentive, quietly melancholic.

## 4. Public Opportunity Summary
Central Shibuya mixed-use complex with gallery space and a cafe; programs exhibitions and community events — proposal-based.

## 5. Existing Fit Notes
A central-Shibuya venue with gallery space and event programming, reachable by proposal rather than a juried call. Location and foot traffic are the concrete draw; whether the space suits your work is something to check against their current shows.

## 6. Key Points
- Gallery + event space in central Shibuya — high foot traffic, proposal-based access.
- Programs both exhibitions and community events — more than one way to show.
- Confirm fees and how proposals are handled before preparing — terms aren't listed.

## 7. Submission / Logistics
- **Deadline:** ongoing — proposal-based
- **Fees:** Confirm with venue
- **Submission page:** https://shibuyacast.jp/
- **Source:** https://shibuyacast.jp/

## 8. Risk / Unknowns
No hard dealbreaker recorded. Manual verification recommended.

## 9. Recommended Next Step
Check shibuyacast.jp for current programming, then message the gallery team (@shibuyacast) to ask how exhibition proposals and fees work.

## 10. Inquiry Need
If submission/contact details are missing, generate a short inquiry email instead of presenting the opportunity as fully verified.

_Last generated: 2026-08-11_
