...
## 6. Chinese register in cached opportunity translations
- **Date:** 2026-09-11
- **File:** `memory/translation_cache.json` (31 occurrences across 9 template strings)
- **What changed:** bureaucratic diction in the venue-tracker templates, replaced with the
  page's actual register — `具名联系人`→`有明确姓名的联系人`, `浏览其线上库存`→`看看网店里的在售商品`,
  `实地察看展示空间，并与负责人洽谈近期展览档期`→`实地看看展示空间，再和负责人谈谈近期的展览档期`,
  plus `确认风格契合度`, `在正式接洽之前`, `观察其策展风格与受众群体` and three others.
  No meaning, condition, venue or action changed — this is register only.
  Source: `_reviews/saffron_naturalness_2026-09-11/`.
- **Why not an engine rule:** the file is written by the translation engine, which calls
  Claude. The "rule" that would produce this output is a better prompt, and a prompt is not
  deterministic — a re-run could reproduce the same officialese, or different officialese.
  There is no assertion that would catch it either: the old strings were valid Chinese,
  just wrong in register for a page written to sound like a person.
- **Consequence:** a future translation pass MAY regenerate these back to machine register.
  That is the accepted risk. It is a cache of machine output, not curated data, so losing
  the patch costs nothing that cannot be re-applied from the review file above.
- **What would actually fix it:** the translation engine's prompt should carry the page's
  voice rules (the same ones in CLAUDE.md — warm, second person on Saffron, no官方 register)
  rather than translating each string in isolation. That is the real engine rule and it is
  not built.
